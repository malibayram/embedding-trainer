...
def collect_samples_streaming(lang_code: str, total_needed: int) -> list[dict]:
    """
    ...
    try:
        ...
        for item in tqdm(ds, desc=f"  Streaming {lang_code}", total=total_needed, leave=False):
            samples.append({
                "lang": lang_code,
...
def split_samples(samples: list[dict], train_n: int, val_n: int, test_n: int) -> tuple:
...
def main():
    print("=" * 70)
    print("WIKIPEDIA 40-LANGUAGE DATASET PREPARATION")
    print("=" * 70)
    ...
    for lang_code, train_n, val_n, test_n in LANGUAGES:
        total_needed = train_n + val_n + test_n
        print(f"[{lang_code}] Collecting {total_needed:,} samples...")
        
        samples = collect_samples_streaming(lang_code, total_needed)
        
        if not samples:
            print(f"  ⚠️  No samples collected for {lang_code}, skipping")
            continue
            
        train, val, test = split_samples(samples, train_n, val_n, test_n)
        
        all_train.extend(train)
        all_val.extend(val)
        all_test.extend(test)
        
        print(f"  ✓ Collected {len(train):,} train, {len(val):,} val, {len(test):,} test")
