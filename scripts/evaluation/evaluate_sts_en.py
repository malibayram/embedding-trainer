#!/usr/bin/env python3
"""
Minimal evaluator for benchmarking models on English STS benchmark.

Uses the sentence-transformers/stsb dataset which contains English sentence pairs
with human similarity scores (0-5 scale).

Usage:
    python evaluate_sts_en.py --model "alibayram/distilled-sentence-transformer2"
    python evaluate_sts_en.py --model "intfloat/multilingual-e5-small"
    
    # Compare multiple models
    python evaluate_sts_en.py --model "model1" "model2" "model3"
"""
...
def evaluate_sts_tr(
    ...
    """
    logger.info(f"Loading sentence-transformers/stsb dataset (split: {split})")
    ...
    scores = dataset["score"] # for english no need to [float(s) / 5.0 for s in dataset["score"]]
...
def compare_models(
    ...
    print("Dataset: sentence-transformers/stsb")
    print("=" * 70)
    ...
    for result in results:
        model_display = result.model_name[:48] + ".." if len(result.model_name) > 50 else result.model_name
        print(f"{model_display:<50} {result.pearson:>8.4f} {result.spearman:>8.4f} {result.processing_time:>8.2f}s")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate models on English STS benchmark (sentence-transformers/stsb)"
    )
    parser.add_argument(
        "--model", "-m",
        nargs="+",
